...
def backoff(function, max_attempts=3, initial_wait=1.0, backoff_factor=2.0):
    ...
    for attempt in range(max_attempts):
        try:
            ...
            print(
                f"Attempt {attempt + 1} failed with {e}. Retrying in {wait_time} seconds."
            )
...
def scrape_and_write_problems(problem_link):
    ...
    try:
        ...
        for i in range(0, len(tds), 3):
            ...
            problem_link_element = backoff(
                lambda: group[1].find_element(By.TAG_NAME, "a")
            )
            solution_link_element = backoff(
                lambda: group[2].find_element(By.TAG_NAME, "a")
            )
            ...
            try:
                ...
                if str(i) in contents["problems"]:
                    print(f"skipping problem {problem_link}")
                    continue
                ...
                print(f'scraped problem {problem_id}; sleeping 10s')
                time.sleep(10)
            except:
...
def scrape_leetcode(problem_link):
    ...
    try:
        ...
        topics = [topic.get_attribute("textContent") for topic in topic_as]
...
scrape_and_write_problems("https://github.com/cnkyrpsgl/leetcode")
